...
class Carpeta:
  ...
  #Funciones de set o agregar/quitar
  ...
  def ver_mensajes(self):
    aux = []
    for mensaje in self.get_mensajes(): #Uso la función get ya establecida en lugar de acceder al atributo
      aux.append(mensaje.get_asunto())
      #print(aux)
    return aux

  def imprimir_mensajes(self, origen = None):
    for mensaje in self.get_mensajes():
...
class ServidorCorreo:
  ...
  def ver_usuariosnombres(self):
    ...
    for usuario in self.get_usuarios():
      aux.append(usuario.get_unombre())
    return aux
  ...
  def enviar_mensaje(self,usuario_emisor, usuario_receptor, asunto, cuerpo):
    ...
    try:
      ...
      #Busco si existen el emisor y receptor en el objeto servidor
      emisor = next((c for c in self.get_usuarios() if c.get_correo() == usuario_emisor.get_correo()), None)
      receptor = next((c for c in self.get_usuarios() if c.get_correo() == usuario_receptor.get_correo()), None)

      if not emisor or not receptor:
        raise ValueError("Emisor o receptor no encontrado en el servidor")
...
#Agregando los usuarios al servidor
...
#Borrando un usuario del servidor
servidor.delete_usuario(2)

#Viendo los nombres de los usuarios existentes en el servidor
servidor.ver_usuariosnombres()